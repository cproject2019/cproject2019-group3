	...
	case 0:
	{
		if (PositionY == 0)
		{
			if (FatherNode->lastOperation == OperationNone)
			{
				for (i = 0;i<2;i++)
				{
					FatherNode->nextNode[i] = tree_malloc_2_child_node();
			...
			{
				FatherNode->nextNode[0] = tree_malloc_2_child_node();
...
Type_NumberNode * tree_malloc_2_child_node(void)
{
	...
	if ((pReturn == NULL) || (pReturn == (Type_NumberNode *)0xCDCDCDCD))
		...
		//printf("Applying memory successfully for child nodes. \n");
		return pReturn;
	return pReturn;
...
int  Createnextnode(Type_NumberNode * TreeHead, int TreeDepth)//接受一个结构体指针叫TreeHead和深度
{
	...
	else
	{
		...
		for (i = 0;i<4;i++)
		{
			...
			{
				if (TreeHead->TreeDepth >= 12)//如果其搜索深度大于12//第一类剪枝
				{
...
int IfFindgoal(Type_NumberNode * Node)//第二类剪枝
{
...
{
	int i, j;
	printf("Please enter the searth depth that you want to searth for Numberslider:");
	scanf_s("%d", &PUBLIC_TreeDepth);
	printf("Please enter your matrix ,examples are as follows:示例如下：\n");
